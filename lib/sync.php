...
if (!function_exists('sync_enqueue_or_fail')) {
    function sync_enqueue_or_fail(PDO $conn, string $entityType, int $entityId, string $actionType = 'upsert', array $options = []): void
    {
        ...
        if (!sync_enqueue_entity_change($conn, $entityType, $entityId, $actionType, $options)) {
...
if (!function_exists('sync_enqueue_delete_or_fail')) {
    function sync_enqueue_delete_or_fail(PDO $conn, string $entityType, array $row, array $options = []): void
    {
        ...
        if (!sync_enqueue_entity_delete($conn, $entityType, $row, $options)) {
...
if (!function_exists('sync_mark_queue_issue')) {
    ...
    {
        $status = in_array($status, ['failed', 'conflict'], true) ? $status : 'failed';
        ...
        $update = $conn->prepare('UPDATE sync_queue SET status = :status, attempts = :attempts, locked_at = NULL, last_error = :last_error, next_attempt_at = :next_attempt_at, updated_at = :updated_at WHERE id = :id');
        return $update->execute([
            'status' => $status,
            'attempts' => $attempts,
...
if (!function_exists('sync_client_run')) {
    function sync_client_run(PDO $conn, array $options = []): array
    {
        ...
        if (!sync_is_enabled() || !sync_is_client() || !sync_schema_ready($conn)) {
            $result['message'] = 'Sync is disabled or schema is missing.';
            return $result;
        }
        ...
        if (empty($rows)) {
            ...
            $result['success'] = true;
            $result['message'] = 'No pending sync items.';
            return $result;
        }
            ...
            if (!(bool) ($response['success'] ?? false) || !is_array($response['body'] ?? null)) {
                ...
                foreach ($remoteResults as $remoteResult) {
                    ...
                    if ($queueStatus === 'synced') {
                        ...
                        sync_mark_queue_issue($conn, $queueId, 'conflict', $message, 1);
                        $result['conflict']++;
                    } else {
        ...
        $result['message'] = $result['processed'] > 0 ? 'Sync run completed.' : 'No items processed.';
        if ($result['synced'] > 0) {
            sync_mark_device_last_sync($conn);
        } else {
            sync_ensure_device_row($conn);
